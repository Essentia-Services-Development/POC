...
//    documentation.
//
//    SetDate()    GeckoDate    set class to a specific date/time
...
//    BOW()        GeckoDate    return first day of week in new GeckoDate
//    EOW()        GeckoDate    return last day of week in new GeckoDate
//    BOM()        GeckoDate    return first day of month in new GeckoDate
//    EOM()        GeckoDate    return last day of month in new GeckoDate
//    Quarter()    Integer            return quarter (1-4) for class
//    BOQ()        GeckoDate    return beginning of quarter in new GeckoDate
//    EOQ()        GeckoDate    return end of quarter in new GeckoDate
//    BOY()        GeckoDate    return beginning of year in new GeckoDate
//    EOY()        GeckoDate    return end of year in new GeckoDate
//    ToString()    GeckoDate    return a formatted date/time string
//    UTC()        GeckoDate    return UTC date/time in new GeckoDate
//
...
//        object        (GeckoDate)        existing date class
//
...
class GeckoDate
{
	...
	function _parseDate($dateTime = '')
	{
		...
		switch (gettype($dateTime))
		{
			...
			case 'integer':
				...
				break;
			case 'object':
				if ('GeckoDate' === get_class($dateTime) || 'DateTime' === get_class($dateTime)) {
	...
	//
	//    Returned value is in a new GeckoDate instance and does
	...
	function BOW($dateTime = '')
	{
		...
		$dt = new GeckoDate($this->TimeStamp());
	...
	function BOQ($quarter = 0, $dateTime = '')
	{
		...
		$dt = new GeckoDate(mktime($this->Hours(),$this->Minutes(),$this->Seconds(),$month,1,$this->Year()));
		return $dt;
	} //BOQ()
...
//    object        object            GeckoDate
//
//    Prerequisites:    requires GeckoDate
//
//    Public member functions. See member function comments for more complete
//    complete descriptions:
//    DateSpanClass()            constructor
//
//    StartDate                (property) return GeckoDate of starting date in span
//    StopDate                (property) return GeckoDate of stopping date in span
//
...
class DateSpanClass
{
	...
	{
		$this->_reset();
		if (!$dateStart) $dateStart = new GeckoDate();
		if (!$dateStop) $dateStop = new GeckoDate();
	...
	function Span($period, $dateStart = '', $dateStop = '')
	{
		...
		//--------------------------------------------------
		if ($dateStart)
			$this->StartDate = new GeckoDate($dateStart);
		if ($dateStop)
			$this->StopDate = new GeckoDate($dateStop);
	...
	function _calcQuarters()
	{
		...
		{
			$dtStart = new GeckoDate($this->StartDate);
			$dtStart = $dtStart->BOQ();
			$dtStop = $this->StopDate;
		}
		else
		{
			$dtStart = new GeckoDate($this->StopDate);
			$dtStart = $dtStart->BOQ();
