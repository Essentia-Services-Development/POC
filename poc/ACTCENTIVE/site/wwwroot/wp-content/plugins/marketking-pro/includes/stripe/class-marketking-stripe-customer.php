...
class WC_Stripe_Customer {
	...
	public function create_customer( $args = array() ) {
		...
		if ( $user ) {
			...
			$description = __( 'Name', 'marketking' ) . ': ' . $billing_first_name . ' ' . $billing_last_name . ' ' . __( 'Username', 'marketking' ) . ': ' . $user->user_login;
	...
	public function add_source( $source_id, $retry = true ) {
		...
		if ( ! empty( $response->error ) ) {
			...
			return new WP_Error( 'error', __( 'Unable to add payment source.', 'marketking' ) );
		}
