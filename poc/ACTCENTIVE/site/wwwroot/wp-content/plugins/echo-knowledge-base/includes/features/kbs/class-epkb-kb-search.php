...
class EPKB_KB_Search {
	...
	/**
	 * Display a search form for:
	 *  a) KB Main Page: core layouts - called by Elegant Layouts, Links Editor, old core layouts
	 *  b) KB Article Page: called by Articles Setup
	 *
	 * @param $kb_config
	 */
	public static function get_search_form( $kb_config ) {
		...
		// handle Modular search (Main Page and Article page); Sidebar Article page is filtered out earlier in EPKB_Articles_Setup
		if ( $kb_config['modular_main_page_toggle'] == 'on' ) {
			EPKB_Modular_Main_Page::search_module( $kb_config );
			return;
		}

		// handle non-Modular search
