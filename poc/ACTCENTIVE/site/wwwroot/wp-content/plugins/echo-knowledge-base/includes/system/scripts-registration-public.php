...
function epkb_frontend_kb_theme_styles_now( $kb_config, $css_file_slug ) {
	...
	if ( in_array( $css_file_slug, [ 'mp-frontend-sidebar-layout', 'ap-frontend-layout' ] ) ) {
		...
		// - is not Advanced search (ASEA uses its own Search box and styles)
		// - is Article structure v2 and first KB version 7.3.0 or higher (v1 or older users have no search on Article Pages)
		if ( $css_file_slug == 'ap-frontend-layout' && $kb_config['modular_main_page_toggle'] == 'on'
			...
			&& EPKB_Articles_Setup::is_article_structure_v2( $kb_config ) && EPKB_Utilities::is_new_user( '7.3.0' ) ) {
