	...
	*/
	...
	public static function get_article_content_and_features( $article, $content, $kb_config ) {
	   ...
	   $v2_suffix = $kb_config['article_content_enable_rows'] == 'on' ? '-v2' : '';
		...
		/**
		 * For the Sidebar Layout we use article page HTML structure. The Sidebar Layout has width controls related to article page.
		 * Article Page width is not required since sidebar layout is nested inside the module row.
		 */
	...
	public static function search_box( $args ) {
		...
		// Sidebar layout on Modular Page will not output search box if Main Page is being shown
		if ( $eckb_is_kb_main_page && $args['config']['kb_main_page_layout'] == EPKB_Layout::SIDEBAR_LAYOUT && $args['config']['modular_main_page_toggle'] == 'on' ) {
			return;
		}
		...
		// setting to hide/show core search box on Article Page (for Modular only)
		if ( $args['config']['modular_main_page_toggle'] == 'on' && $args['config']['article_search_sync_toggle'] != 'on' && $args['config']['article_search_toggle'] != 'on' ) {
			return;
		}
		...
		// v1 or older users have no search on Article Pages  TODO remove
		if ( ! self::is_article_structure_v2( $args['config'] ) || ! EPKB_Utilities::is_new_user( '7.3.0' ) ) {
			return;
		}

		// output core search form
