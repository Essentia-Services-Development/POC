...
class EPKB_Layouts_Setup {
	...
	public static function get_kb_page_output_hook( $content, $the_content_filter_call = true ) {
		...
		if ( $kb_config['kb_article_page_layout'] == EPKB_Layout::SIDEBAR_LAYOUT && ! EPKB_Articles_Setup::is_article_structure_v2( $kb_config ) ) {
	...
	public static function output_main_page( $kb_config, $is_builder_on=false, $article_seq=array(), $categories_seq=array() ) {
		...
		if ( self::is_elay_layout( $layout ) ) {
			...
			if ( $layout == EPKB_Layout::SIDEBAR_LAYOUT && EPKB_Articles_Setup::is_article_structure_v2( $kb_config ) ) {
			...
			} else if ( ! $is_modular ) {  // Grid Layout or V1 Sidebar Layout
		...
		// handle Core layouts and Modular page with all layouts except Sidebar
