<?php
	global $pmpro_review, $pmpro_paypal_token, $wpdb, $pmpro_msg, $pmpro_msgt, $pmpro_levels, $tospage, $pmpro_error_fields;
...
<form id="pmpro_form" class="pmpro_form" action="<?php if(!empty($_REQUEST['review'])) echo pmpro_url("checkout", "?level=" . $pmpro_level->id); ?>" method="post">
	...
	<?php if($pmpro_review) { ?>
		<p><?php echo __('Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'pmpro');?></p>
	<?php } ?>
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-name"><?php echo __('Membership Level', 'pmpro');?></span>
				<?php if(count($pmpro_levels) > 1) { ?><span class="pmpro_thead-msg"><a href="<?php echo pmpro_url("levels"); ?>"><?php echo __('change', 'pmpro');?></a></span><?php } ?>
			</th>
		</tr>
	</thead>
	<tbody>
			...
			<td>
				...
				<?php if($pmpro_show_discount_code) { ?>
					...
					<?php if($discount_code && !$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><a id="other_discount_code_a" href="#discount_code"><?php echo __('Click here to change your discount code', 'pmpro');?></a>.</p>
					<?php } elseif(!$pmpro_review) { ?>
						<p id="other_discount_code_p" class="pmpro_small"><?php echo __('Do you have a discount code?', 'pmpro');?> <a id="other_discount_code_a" href="#discount_code"><?php echo __('Click here to enter your discount code', 'pmpro');?></a>.</p>
					<?php } elseif($pmpro_review && $discount_code) { ?>
						<p><strong><?php echo __('Discount Code', 'pmpro');?>:</strong> <?php echo $discount_code?></p>
					<?php } ?>
		...
		<tr id="other_discount_code_tr" style="display: none;">
			<td>
				<div>
					<label for="other_discount_code"><?php echo __('Discount Code', 'pmpro');?></label>
					...
					<input type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php echo __('Apply', 'pmpro');?>" />
				</div>
			</td>
		</tr>
		<?php } ?>
	...
	<script>
		...
		jQuery('#other_discount_code_a').attr('href', '#');
		jQuery('#other_discount_code_a').click(function(e) {
			e.preventDefault();
	...
	<thead>
		<tr>
			<th>
				<span class="pmpro_thead-name"><?php echo __('Account Information', 'pmpro');?></span>
				<span class="pmpro_thead-msg"><?php echo __('Already have an account?', 'pmpro');?> <a href="<?php echo wp_login_url(pmpro_url("checkout", "?level=" . $pmpro_level->id))?>"><?php echo __('Log in here', 'pmpro');?></a></span>
			</th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
				<div>
					<label for="username"><?php echo __('Username', 'pmpro');?></label>
				...
				<div>
					<label for="password"><?php echo __('Password', 'pmpro');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="password2"><?php echo __('Confirm Password', 'pmpro');?></label>
				...
				<div>
					<label for="bemail"><?php echo __('Email Address', 'pmpro');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="bconfirmemail"><?php echo __('Confirm Email Address', 'pmpro');?></label>
				...
				<div class="pmpro_hidden">
					<label for="fullname"><?php echo __('Full Name', 'pmpro');?></label>
					<input id="fullname" name="fullname" type="text" class="input <?php echo pmpro_getClassForField("fullname");?>" size="30" value="" /> <strong><?php echo __('LEAVE THIS BLANK', 'pmpro');?></strong>
				</div>
	...
	<?php } elseif($user_id && !$pmpro_review) { ?>

		<p id="pmpro_account_loggedin">
			<?php printf(__('You are logged in as <strong>%s</strong>.', 'pmpro'), $u->get_username()); ?>
		</p>
	<?php } ?>
	...
	<?php if(pmpro_getGateway() == "paypal" && empty($pmpro_review)) { ?>
		...
		<thead>
			<tr>
				<th><?php echo __('Choose your Payment Method', 'pmpro');?></th>
			</tr>
		</thead>
		<tbody>
			<tr>
				<td>
					<div>
						<span class="gateway_paypal">
							...
							<a href="#" class="pmpro_radio" onclick="return false;"><?php echo __('Check Out with a Credit Card Here', 'pmpro');?></a>
						</span>
						<span class="gateway_paypalexpress">
							...
							<a href="#" class="pmpro_radio" onclick="return false;"><?php echo __('Check Out with PayPal', 'pmpro');?></a>
						</span>
					</div>
				</td>
			</tr>
	...
	<thead>
		<tr>
			<th><?php echo __('Billing Address', 'pmpro');?></th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td>
				<div>
					<label for="bfirstname"><?php echo __('First Name', 'pmpro');?></label>
				...
				<div>
					<label for="blastname"><?php echo __('Last Name', 'pmpro');?></label>
				...
				<div>
					<label for="baddress1"><?php echo __('Address 1', 'pmpro');?></label>
				...
				<div>
					<label for="baddress2"><?php echo __('Address 2', 'pmpro');?></label>
				...
				?>
					<div>
						<label for="bcity"><?php echo __('City', 'pmpro');?></label>
					...
					<div>
						<label for="bstate"><?php echo __('State', 'pmpro');?></label>
					...
					<div>
						<label for="bzipcode"><?php echo __('Postal Code', 'pmpro');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="bcity_state_zip"><?php echo __('City, State Zip', 'pmpro');?></label>
				...
				?>
				<div>
					<label for="bcountry"><?php echo __('Country', 'pmpro');?></label>
				...
				?>
				<div>
					<label for="bphone"><?php echo __('Phone', 'pmpro');?></label>
				...
				<?php
					if($user_id)
					{
						if(!$bemail && $u->get_email())
							$bemail = $u->get_email();
						if(!$bconfirmemail && $u->get_email())
							$bconfirmemail = $u->get_email();
					}
				?>
				<div>
					<label for="bemail"><?php echo __('Email Address', 'pmpro');?></label>
				...
				<?php
					...
					?>
					<div>
						<label for="bconfirmemail"><?php echo __('Confirm Email', 'pmpro');?></label>
						<input id="bconfirmemail" name="bconfirmemail" type="<?php echo ($pmpro_email_field_type ? 'email' : 'text'); ?>" class="input <?php echo pmpro_getClassForField("bconfirmemail");?>" size="30" value="<?php echo esc_attr($bconfirmemail)?>" />
	...
	<?php
		...
		<thead>
			<tr>
				<th>
					<span class="pmpro_thead-name"><?php echo __('Payment Information', 'pmpro');?></span>
				...
				<td>
					...
					<?php
						...
						<div class="pmpro_payment-card-type">
							<label for="CardType"><?php echo __('Card Type', 'pmpro');?></label>
					...
					<div class="pmpro_payment-account-number">
						<label for="AccountNumber"><?php echo __('Card Number', 'pmpro');?></label>
					...
					<div class="pmpro_payment-expiration">
						<label for="ExpirationMonth"><?php echo __('Expiration Date', 'pmpro');?></label>
					...
					<div class="pmpro_payment-cvv">
						<label for="CVV"><?php echo __('CVV', 'pmpro');?></label>
						<input class="input" id="CVV" name="CVV" type="text" size="4" value="<?php if(!empty($_REQUEST['CVV'])) { echo esc_attr($_REQUEST['CVV']); }?>" class=" <?php echo pmpro_getClassForField("CVV");?>" />  <small>(<a href="#" onclick="window.open('<?php echo pmpro_https_filter($cvv_template); ?>,'cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475'); return false;"><?php echo __("what's this?", 'pmpro');?></a>)</small>
					</div>
					<?php
						}
					?>
					...
					<div class="pmpro_payment-discount-code">
						<label for="discount_code"><?php echo __('Discount Code', 'pmpro');?></label>
						...
						<input type="button" id="discount_code_button" name="discount_code_button" value="<?php echo __('Apply', 'pmpro');?>" />
	...
	<div class="pmpro_submit">
		<?php if($pmpro_review) { ?>
			...
			<span id="pmpro_submit_span">
				...
				<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php echo __('Complete Payment', 'pmpro');?> &raquo;" />
			</span>
		...
		<?php } else { ?>
			...
			<?php
				...
				<span id="pmpro_submit_span">
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php if($pmpro_requirebilling) { echo __('Submit and Check Out', 'pmpro'); } else { echo __('Submit and Confirm', 'pmpro');}?> &raquo;" />
				</span>
				<?php
				}
			?>
