 ...
 *
 */
if ( ! class_exists( 'PVFWOF_Shortcoder' ) ) {
  class PVFWOF_Shortcoder extends PVFWOF_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_pvfwof-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        $name = str_replace( '_', '-', sanitize_title( $this->unique ) );

        PVFWOF::$shortcode_instances[] = wp_parse_args( array( 'name' => 'pvfwof/'. $name, 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( PVFWOF::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'PVFWOF', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'PVFWOF_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      if( ! wp_script_is( 'pvfwof' ) ) {
        return;
      }

      $class        = ( $this->args['class'] ) ? ' '. esc_attr( $this->args['class'] ) : '';
      $has_select   = ( count( $this->pre_tabs ) > 1 ) ? true : false;
      $single_usage = ( ! $has_select ) ? ' pvfwof-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="pvfwof-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui pvfwof-modal pvfwof-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'pvfwof_shortcode_nonce' ) ); ?>">
          ...
          <div class="pvfwof-modal-table-cell">
            ...
            <div class="pvfwof-modal-inner">
              <div class="pvfwof-modal-title">
                ...
                <div class="pvfwof-modal-close"></div>
              </div>
              <?php

                echo '<div class="pvfwof-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="pvfwof-modal-insert-wrapper hidden"><a href="#" class="button button-primary pvfwof-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'pvfwof_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              PVFWOF::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            $button_title    = ( ! empty( $section['button_title'] ) ) ? ' '. $section['button_title'] : esc_html__( 'Add New', 'pvfwof' );
            $inner_shortcode = ( ! empty( $section['group_shortcode'] ) ) ? $section['group_shortcode'] : $shortcode;

            echo '<div class="pvfwof--repeatable">';

              echo '<div class="pvfwof--repeat-shortcode">';

                echo '<div class="pvfwof-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  PVFWOF::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="pvfwof--repeat-button-block"><a class="button pvfwof--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
        ...
        echo '<div class="pvfwof-field pvfwof-error-text">'. esc_html__( 'Error: Invalid nonce verification.', 'pvfwof' ) .'</div>';
      }
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'PVFWOF_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( pvfwof_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'PVFWOF_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'pvfwof-gutenberg-block', PVFWOF::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'pvfwof-gutenberg-block', 'pvfwof_gutenberg_blocks', PVFWOF::$shortcode_instances );

      foreach ( PVFWOF::$shortcode_instances as $block ) {

        register_block_type( $block['name'], array(
          'editor_script' => 'pvfwof-gutenberg-block',
        ) );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( PVFWOF::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary pvfwof-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. esc_html( $value['button_title'] ) .'</a>';
      }
