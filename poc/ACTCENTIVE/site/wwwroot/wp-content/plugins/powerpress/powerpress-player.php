	...
	if( preg_match('/(content|protected|ins|mc)\.blubrry\.com/', $media_url) )
	{
		...
		if ($GeneralSettings['player'] == 'blubrrymodern') {
            ...
            if (!empty($EpisodeData['sample'])) {
                return '<iframe sandbox="allow-scripts allow-popups allow-forms" src="' . $url . '" scrolling="yes" width="100%" height="' . htmlspecialchars($playerSettings['transcriptspacing']) . '" frameborder="0" id="' . $playerID . '" class="blubrryplayer" title="' . $iframeTitle . '"></iframe>';
            } else {
                return '<iframe src="' . $url . '" scrolling="yes" width="100%" height="'. htmlspecialchars($playerSettings['transcriptspacing']).'" frameborder="0" id="' . $playerID . '" class="blubrryplayer" title="' . $iframeTitle . '"></iframe>';
            }
        } else {
