(function ($) {
	...
	// input handler
	$container
		.on('input', '.ps-js-field input[type=text]', function () {
		...
		.find('.ps-js-field input[type=text]')
		.each(function () {
	...
	$container.on('click', '.ps-js-field .ps-js-save', function () {
		...
		if (params.prop === 'validation') {
			...
			if (oMatch) {
				if (params.value.trim() === '' || isNaN(params.value) || +params.value < 0) {
					...
					$paircheck = $tab.find(
						'[data-prop-key="' + pairkey.replace('_value', '') + '"]'
					);
	...
	var updateFieldVisibility = _.debounce(function (id, status) {
		...
		updateFieldXHR[id] = peepso.postJson(
			'adminConfigFields.set_admin_box_status',
	...
	// make sure user_registration_only field is unchecked whenever user_registration unchecked
	$container.on('click', 'input[data-prop-name=user_registration]', function (e) {
		console.log('user_registration');

		var $tab = $(this).closest('.ps-tab__content'),
			$inRegistrationOnly = $tab.find('input[data-prop-name=user_registration_only]'),
			$adminCanEdit = $tab.find('input[data-prop-name=user_admin_editable_only]'),
			$adminCanSee = $tab.find('input[data-prop-name=user_admin_visible_only]');

		if (this.checked) {
			$adminCanEdit.is(':checked') && $adminCanEdit.trigger('click');
			$adminCanSee.is(':checked') && $adminCanSee.trigger('click');
		} else {
			$inRegistrationOnly.is(':checked') && $inRegistrationOnly.trigger('click');
		}
	});

	// make sure user_registration field is checked whenever user_registration_only checked
	$container.on('click', 'input[data-prop-name=user_registration_only]', function (e) {
		console.log('user_registration_only');

		var $tab = $(this).closest('.ps-tab__content'),
			$inRegistration = $tab.find('input[data-prop-name=user_registration]'),
			$adminCanEdit = $tab.find('input[data-prop-name=user_admin_editable_only]'),
			$adminCanSee = $tab.find('input[data-prop-name=user_admin_visible_only]');

		if (this.checked) {
			$inRegistration.is(':checked') || $inRegistration.trigger('click');
			$adminCanEdit.is(':checked') && $adminCanEdit.trigger('click');
			$adminCanSee.is(':checked') && $adminCanSee.trigger('click');
		} else {
		}
	...
	$container.on('click', 'input[data-prop-name=user_admin_editable_only]', function (e) {
		console.log('user_admin_editable_only');
	...
	$container.on('click', 'input[data-prop-name=user_admin_visible_only]', function (e) {
		console.log('user_admin_visible_only');
