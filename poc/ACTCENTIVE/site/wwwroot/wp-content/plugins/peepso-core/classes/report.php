...
//			$user->set_role('peepso_ban');
	 ...
	 * @return mixed Returns the number of rows deleted or FALSE on error.
	 */
	public function unpublish_report($rep_id, $is_automatic = FALSE)
	{
		global $wpdb;
		...
		if (!is_null($report) && wp_update_post(array('ID' => $report->rep_external_id, 'post_status' => 'pending'))) {
			if (!$is_automatic) {
				...
				return $this->soft_delete_reports($report);
			}
		}
	...
	/**
	 * Sets a posts status to publish and deletes the report.
	 * @param  int $rep_id The report ID.
	 * @return mixed Returns the number of rows deleted or FALSE on error.
	 */
	public function republish_report($rep_id)
	{
		global $wpdb;
		...
		if (!is_null($report) && wp_update_post(array('ID' => $report->rep_external_id, 'post_status' => 'publish'))) {
			return $this->delete_reports($report);
		}
	...
	/**
	 * Soft Deletes the report from the 'peepso_report' table with update rep_status to 1.
	 * @param  object $report The report object from the database.
	 * @return mixed Returns the number of rows deleted or FALSE on error.
	 */
	public function soft_delete_reports($report)
	{
		global $wpdb;

        if (!is_null($report))
        {
            return $wpdb->update(self::get_table_name(), 
        		array(
        			'rep_status' => 1,
        		),
                array
