...
import peepsodata, { group as groupData } from 'peepsodata';
...
const COVER_UPLOAD_URL = `${peepsodata.ajaxurl_legacy}groupajax.cover_upload?cover`;
const COVER_TEXT_ERROR_FILETYPE = groupData.text_error_filetype;
const COVER_TEXT_ERROR_FILESIZE = groupData.text_error_filesize;
...
$(function () {
	...
	function rotate(e) {
		...
		let params = $.extend(
			...
			{ group_id: GROUP_ID, _wpnonce: COVER_NONCE }
		);

		ajax.post('groupajax.cover_rotate', params).then(json => {
			if (json.success) {
				let imgCover = json.data.image_url;
