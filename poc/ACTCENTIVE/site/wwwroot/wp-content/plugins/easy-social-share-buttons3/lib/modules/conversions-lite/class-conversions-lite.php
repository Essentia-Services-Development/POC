...
class ESSBSocialConversionsLite {
    ...
    public function conversion_tracking () {
        ...
        if ($essb_conversion_logging == 'true') {
            ...
            if (! $ab_stats || ! is_array($ab_stats)) {
                $ab_stats = array ( 
                    'totals' => array (), 'dates' => array () 
                );
            }
            
            if (! isset($ab_stats['dates'][$log_date])) {
                $ab_stats['dates'][$log_date] = array ( 
                    'positions' => array (), 'networks' => array () 
                );
            }
            ...
            if (isset($data->positions)) {
                ...
                foreach ($data->positions as $key => $value) {
                    /**
                     * Prevent further further save if the position is not set
                     */                    
                    if (!$this->valid_position($key)) {
                        continue;
                    }
                    
                    
                    if (! isset($ab_stats['totals']['positions'][$key])) {
                        $ab_stats['totals']['positions'][$key] = array ( 
                            'views' => intval($value), 'clicks' => 0 
                        );
                    }
                    else {
                    ...
                    if (! isset($ab_stats['dates'][$log_date]['positions'][$key])) {
                        $ab_stats['dates'][$log_date]['positions'][$key] = array ( 
                            'views' => intval($value), 'clicks' => 0 
                        );
                    }
                    else {
            ...
            if (isset($data->networks)) {
                ...
                foreach ($data->networks as $key => $value) {
                    ...
                    /**
                     * Prevent further save for unknown networks
                     */
                    if (!$this->valid_network($key)) {
                        continue;
                    }
                    
                    if (! isset($ab_stats['totals']['networks'][$key])) {
                        $ab_stats['totals']['networks'][$key] = array ( 
                            'views' => intval($value), 'clicks' => 0 
                        );
                    }
                    else {
                    ...
                    if (! isset($ab_stats['dates'][$log_date]['networks'][$key])) {
                        $ab_stats['dates'][$log_date]['networks'][$key] = array ( 
                            'views' => intval($value), 'clicks' => 0 
                        );
                    }
                    else {
        ...
        if ($essb_conversion_tracking == 'true') {
            ...
            /**
             * Prevent further data save if unknow data is passed
             */
            if (!$this->valid_network($service) || !$this->valid_position($position)) {
                die();
                return;
            }
            ...
            if (isset($position) && $position != '') {
                ...
                if (! isset($ab_stats['totals']['positions'][$position])) {
                    $ab_stats['totals']['positions'][$position] = array ( 
                        'views' => 0, 'clicks' => 1 
                    );
                }
                else {
                ...
                if (! isset($ab_stats['dates'][$log_date]['positions'][$position])) {
                    $ab_stats['dates'][$log_date]['positions'][$position] = array ( 
                        'views' => 0, 'clicks' => 1 
                    );
                }
                else {
            ...
            if (isset($service) && $service != '') {
                ...
                if (! isset($ab_stats['totals']['networks'][$service])) {
                    $ab_stats['totals']['networks'][$service] = array ( 
                        'views' => 0, 'clicks' => 1 
                    );
                }
                else {
                ...
                if (! isset($ab_stats['dates'][$log_date]['networks'][$service])) {
                    $ab_stats['dates'][$log_date]['networks'][$service] = array ( 
                        'views' => 0, 'clicks' => 1 
                    );
                }
                else {
    ...
    public function register_tracking () {
		...
		function essbConversionsLiteLog() {
			...
			var essbab_url = "' . esc_url($update_url) . '";
    ...
    /**
     * Check if it is a valid position key
     * 
     * @param string $key
     * @return boolean
     */
    private function valid_position($position = '') {
        $is_valid = false;
        
        $content_positions = essb_avaliable_content_positions(true);
        $button_positions = essb_available_button_positions(true);        
        $mobile_positions = essb_available_button_positions_mobile();
    ...
    /**
     * Validate network and prevent storage of invalid network keys
     * 
     * @param string $key
     * @return boolean
     */
    private function valid_network($key = '') {
        ...
        $all_networks[] = 'print_friendly';
        
        return in_array($key, $all_networks);
        
    }
}

/**
 * Calling the main plugin loader
 */
ESSB_Factory_Loader::activate_instance('conversions-lite', 'ESSBSocialConversionsLite');