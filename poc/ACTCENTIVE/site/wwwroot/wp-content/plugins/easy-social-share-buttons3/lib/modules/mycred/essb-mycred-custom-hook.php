...
function register_essb_hook_in_mycred( $installed ) {
	...
	$installed['essb'] = array(
			'title'       => esc_html__( 'Points for click on share buttons via the Easy Social Share Buttons for WordPress plugin', 'essb' ),
			'description' => esc_html__( 'This hook awards / deducts points for click on share buttons via the Easy Social Share Buttons for WordPress plugin.', 'essb' ),
...
class myCRED_Hook_ESSB extends myCRED_Hook {
	...
	public function add_reference($references) {
		$references['essb_clicks'] = esc_html__('Social Sharing (Easy Social Share Buttons)', 'essb');
		return $references;
	}
	...
	public function parse_custom_tags( $content, $log_entry ) {
		...
		if ( isset( $data['link_title'] ) ) {
			...
			$content = str_replace( '%link_title%',  '<a href="'.esc_url($data['link_url']).'" target="_blank">'.esc_attr($data['link_title']).'</a>', $content );
		}
		if ( isset( $data['link_service'] ) )
			$content = str_replace( '%service%',  '<b>'.esc_attr($data['link_service']).'</b>', $content );
	...
	public function preferences() {
		...
		?>
<label class="subheader" for="<?php echo esc_attr($this->field_id( 'creds' )); ?>"><?php esc_html_e( 'Points', 'essb' ); ?></label>
<ol>
	<li>
		<div class="h2"><input type="text" name="<?php echo esc_attr($this->field_name( 'creds' )); ?>" id="<?php echo esc_attr($this->field_id( 'creds' )); ?>" value="<?php echo $this->core->format_number( $prefs['creds'] ); ?>" size="8" /></div>
	</li>
	<li>
		<label for="<?php echo esc_attr($this->field_id( 'limit' )); ?>"><?php esc_html_e( 'Limit', 'essb' ); ?></label>
		<?php echo $this->hook_limit_setting( $this->field_name( 'limit' ), $this->field_id( 'limit' ), $prefs['limit'] ); ?>
	</li>
	<li class="empty">&nbsp;</li>
	<li>
		<label for="<?php echo esc_attr($this->field_id( 'log' )); ?>"><?php esc_html_e( 'Log template', 'essb' ); ?></label>
