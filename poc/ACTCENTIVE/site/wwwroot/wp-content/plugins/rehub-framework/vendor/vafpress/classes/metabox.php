...
class VP_Metabox extends WPAlchemy_MetaBox
{
	...
	function _render_repeating_group($group)
	{
		...
		foreach ($group['groups'] as $g)
		{
			...
			$html .= '<div class="vp-wpa-group-heading"><a href="#" class="vp-wpa-group-title">' . $icon . $group['title'] . '</a><a href="#" class="dodelete vp-wpa-group-remove" title="'. esc_html__('Remove', 'rehub-framework') .'"><i class="rhicon rhi-times"></i> '. esc_html__('Remove', 'rehub-framework') .'</a></div>';
			$html .= '<div class="vp-controls' . ((!$is_first) ? ' vp-hide' : '') . '">';
