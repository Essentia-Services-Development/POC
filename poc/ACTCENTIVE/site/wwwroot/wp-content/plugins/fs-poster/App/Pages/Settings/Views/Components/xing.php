...
<div class="fsp-settings-row">
    <div class="fsp-settings-col">
        ...
        <div class="fsp-settings-label-subtext"><?php echo fsp__( 'Link card view – shares the post link and the custom message. Xing fetches the post as a link card view;<br>Only custom message – shares the custom message without any image or link;<br>Featured image - Shares the featured image and the custom message.', [], FALSE ); ?></div>
    </div>
    <div class="fsp-settings-col">
        <select name="fs_xing_posting_type" id="fspXingPostingType" class="fsp-form-select">
            <option value="1" <?php echo Helper::getOption( 'xing_posting_type', '1' ) == '1' ? 'selected' : ''; ?>><?php echo fsp__( 'Link card view' ); ?></option>
            <option value="2" <?php echo Helper::getOption( 'xing_posting_type', '1' ) == '2' ? 'selected' : ''; ?>><?php echo fsp__( 'Only custom message' ); ?></option>
            <option value="3" <?php echo Helper::getOption( 'xing_posting_type', '1' ) == '3' ? 'selected' : ''; ?>><?php echo fsp__( 'Featured image' ); ?></option>
        </select>
    </div>
</div>
<div class="fsp-settings-row">
    <div class="fsp-settings-col">
        <div class="fsp-settings-label-text"><?php echo fsp__( 'Post Visibility' ); ?></div>
        <div class="fsp-settings-label-subtext"><?php echo fsp__( 'Select the post visibility option for accounts. Please note that the option does not apply to Groups and only public posts can be shared or seen by anyone.', [], FALSE ); ?></div>
    </div>
    <div class="fsp-settings-col">
        <select name="fs_xing_post_visibility" id="fspXingPostVisibility" class="fsp-form-select">
            <option value="1" <?php echo Helper::getOption( 'xing_post_visibility', '1' ) == '1' ? 'selected' : ''; ?>><?php echo fsp__( 'Public' ); ?></option>
            <option value="2" <?php echo Helper::getOption( 'xing_post_visibility', '1' ) == '2' ? 'selected' : ''; ?>><?php echo fsp__( 'Contacts only' ); ?></option>
            <option value="3" <?php echo Helper::getOption( 'xing_post_visibility', '1' ) == '3' ? 'selected' : ''; ?>><?php echo fsp__( 'Only Contacts in my city' ); ?></option>
        </select>
    </div>
</div>
	...
	<div class="fsp-settings-collapse">
		<div class="fsp-settings-col">
			...
			<div class="fsp-custom-post" data-preview="fspCustomPostPreview2">
				<textarea name="fs_post_text_message_xing" class="fsp-form-textarea"><?php echo esc_html( Helper::getOption( 'post_text_message_xing', "{content_full}" ) ); ?></textarea>
